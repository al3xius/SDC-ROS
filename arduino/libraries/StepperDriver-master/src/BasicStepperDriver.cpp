...
void BasicStepperDriver::begin(short rpm, short microsteps){
    ...
    digitalWrite(step_pin, HIGH); //changed

    if IS_CONNECTED(enable_pin){
        pinMode(enable_pin, OUTPUT);
        digitalWrite(enable_pin, LOW); // disable //changed
    }
...
void BasicStepperDriver::startMove(long steps){
    ...
    if (steps_remaining){
        ...
        // set up new move
        dir_state = (steps >= 0) ? LOW : HIGH; //changed
        last_action_end = 0;
...
long BasicStepperDriver::nextAction(void){
    if (steps_remaining > 0){
        ...
        digitalWrite(step_pin, LOW); //changed
        unsigned m = micros();
...
void BasicStepperDriver::enable(void){
    if IS_CONNECTED(enable_pin){
        digitalWrite(enable_pin, HIGH); //changed
    }
}

void BasicStepperDriver::disable(void){
    if IS_CONNECTED(enable_pin){
        digitalWrite(enable_pin, LOW); //changed
    }
}
