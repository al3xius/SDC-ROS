...
class StateMachine():
    ...
    def arduCallback(self, arduinoIn):
        ...
        voltage = interp(arduinoIn.analog[self.batteryPin], [
                         0, 1015], [0, 5]) * self.batteryFactor
        ...
        # get gaspeddal
        self.gasPedal = limitValue(interp(arduinoIn.analog[self.gasPedalPin], [
                                   self.gasPedalMin, self.gasPedalMax], [0, 100]), 0, 100)
    ...
    def joyCallback(self, Joy):
        ...
        # reset after emergency break
        if Joy.buttons[12] and self.mode == "break":
            self.mode = "manual"
            self.pervMode = "manual"

        # hold for cruise control
        if Joy.buttons[9] and self.mode != "cruise":
            self.mode = "cruise"
        elif self.mode == "cruise":
            self.mode = "manual"

        self.joyBreaking = limitValue(
            interp(abs(limitValue(Joy.axes[3], -1, 0)), [0, 1], [0, 100]), 0, 100)
