...
class Detector:
    ...
    def __init__(self):
        self.image_pub = rospy.Publisher("/objectDedector/overlayImage",Image, queue_size=1)
        self.object_pub = rospy.Publisher("/objectDedector/objects", Detection2DArray, queue_size=1)
        self.bridge = CvBridge()
        self.image_sub = rospy.Subscriber("/usb_cam/image_raw", Image, self.image_cb, queue_size=1, buff_size=2**24)
...
def main(args):
    rospy.init_node('objectDedector', anonymous=True)
    obj=Detector()
    try:
        rospy.spin()
